...
void PIDClass::control(float u){
  _u = u;
  if(u != constrain(u, -100.0, 100.0)){
    Serial.println("u is not in range");
    u = constrain(u, -100.0, 100.0);
  }
...
void PIDClass::calculate_velocity(){
  ...
  _previous_time = _current_time;
  _current_velocity = ((_position_encoder - _position_temp)/_dt) * _pulseToRPM; 
...
void PIDClass::pi(float setpoint){
  calculate_velocity(); // undecided whether i should run this here
  float error = setpoint - _current_velocity;  // calculate error
  _integral += error*_dt;
  _integral = constrain(_integral, -100.0/_ki, 100.0/_ki);
  _u = constrain(_kp * error + _ki*_integral, -100.0, 100.0);
